#' Simulate average unfished biomass using functional
#'
#' This function simulates the average unfished biomass of herring using a
#' user-specified functional spawner-recruit relationship such as ricker or
#' beverton-holt. Requires data including weight-at-age, proportion of stock
#' that are mature, and a constant survival rate.
#'
...
#' @param model_type Character string corresponding to the name of a
#' stock-recruit model existing in the users global environment.
...
B_sim_functional <- function(N_t = NULL, recruits, spawners, mean_wt,
                            maturity, S, model_type, num_years = 5000,
                            max_iters = 10000){
  ...
  while(!convergence){
    ...
    # predicts age-3 recruits from B[y-3] spawner biomass using sr model
    N_t$age3[t] <- do.call(what = model_type, args = list(N_t[t-3, "B_tonnes"]))
