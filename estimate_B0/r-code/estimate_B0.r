...
# Outputs: 
#   - data/final_strata.rda: the sampling strata as identified by kmeans
#   - data/raw_results_many_simulations_asa2023.txt: huge file containing combined
#     simulation results
...
saveRDS(sr, file = "estimate_B0/data/final_strata.rda")
...
# write.table(raw_results, "estimate_B0/data/raw_results_many_simulations_asa2023.txt",
...
combined_hist <- ggplot() +
  ...
  annotate("text", x = 5e5, y = .055,
           color = cols[2], size = 5,
           label = bquote("Median" ~ B[0] * ":" ~  .(format(round(quants_combined[3]), big.mark = ",")) ~ "tons")) +
  scale_x_continuous(label = scales::label_comma()) +
  scale_y_continuous(label = scales::percent) +
  theme_bw() 

ggsave("estimate_B0/plots/combined_histogram.png", combined_hist, width = 6, height = 3.5)
